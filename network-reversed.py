...
batch_size = 100
n_epochs = 10
y = tf.placeholder('float', [None, in_classes])
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            for _ in range(int(mnist.train.num_examples / batch_size)):
                ...
                print(epoch_y, epoch_x)
                _, c = sess.run([optimizer, cost], feed_dict={y: epoch_x, x: epoch_y})
                epoch_loss += c
            print('Epoch', epoch, 'completed out of', n_epochs, 'loss:',epoch_loss)
            correct = tf.equal(tf.argmax(prediction, 1), tf.argmax(y, 1))
            accuracy = tf.reduce_mean(tf.cast(correct,'float'))
            print('Accuracy:', accuracy.eval({y:mnist.test.images, x:mnist.test.labels}))
        saver.save(sess, './cp-rev/mnist.ckpt')
